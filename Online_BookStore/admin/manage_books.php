...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_book'])) {
    $book_id_to_delete = intval($_POST['book_id']);
    
    
    $delete_stmt = $conn->prepare("DELETE FROM Books WHERE book_id = ?");
    $delete_stmt->bind_param("i", $book_id_to_delete);
    
    if ($delete_stmt->execute()) {
        $message = "Book deleted successfully!";
    } else {
        
        $error = "Could not delete book. It might be linked to existing orders.";
    }
    $delete_stmt->close();
}


$query = "SELECT * FROM book_details ORDER BY book_id DESC";
    ...
    <div class="main-content">
        ...
        <table>
            ...
            <tbody>
                <?php if ($books_result && $books_result->num_rows > 0): ?>
                    <?php while($book = $books_result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <span style="color: <?php echo ($book['stock'] < 5) ? 'red' : 'black'; ?>; font-weight: bold;">
                                    <?php echo $book['stock']; ?>
                                </span>
                            </td>
                            <td>$<?php echo number_format($book['price'], 2); ?></td>
                            <td><?php echo htmlspecialchars($book['cover_type']); ?></td>
                            <td class="actions">
                                <a href="edit_book.php?id=<?php echo $book['book_id']; ?>" class="btn-edit">Edit</a>
                                
                                <form action="manage_books.php" method="POST" onsubmit="return confirm('Are you sure you want to delete this book?');">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" style="text-align: center;">No books found in the database.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
