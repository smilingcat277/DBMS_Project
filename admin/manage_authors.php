...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_author'])) {
    $author_id_to_delete = intval($_POST['author_id']);
    
    $delete_stmt = $conn->prepare("DELETE FROM Authors WHERE author_id = ?");
    $delete_stmt->bind_param("i", $author_id_to_delete);
    
    if ($delete_stmt->execute()) {
        $message = "Author deleted successfully!";
    } else {
        
        $error = "Cannot delete author. They are currently linked to books in the database. Please delete or reassign their books first.";
    }
    $delete_stmt->close();
}


$query = "SELECT author_id, first_name, last_name FROM Authors ORDER BY author_id ASC";
...
<body>
    ...
    <div class="main-content">
        <div class="header">
            ...
            <a href="add_author.php" class="btn-add">+ Add New Author</a>
        </div>
                ...
                <?php if ($authors_result && $authors_result->num_rows > 0): ?>
                    <?php while($author = $authors_result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td class="actions">
                                <a href="edit_author.php?id=<?php echo $author['author_id']; ?>" class="btn-edit">Edit</a>
                                
                                <form action="manage_authors.php" method="POST" onsubmit="return confirm('Are you sure you want to delete this author?');">
                                    <input type="hidden" name="author_id" value="<?php echo $author['author_id']; ?>">
                                    <button type="submit" name="delete_author" class="btn-delete">Delete</button>
                                </form>
                            </td>
                        </tr>
                    <?php endwhile; ?>
